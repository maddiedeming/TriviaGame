...
var bonusQuestions = [	
	{
		question: "Also for anyone that has money they know the first rule is to use other people's money.",
		correctAnswer: "Kanye West",
		src: "#",
		//profileImg: "assets/images/kanye-west-profile.jpeg",
		twitterHandle: "@KanyeWest"
	...
	{
		question: "I have never seen a thin person drinking Diet Coke.",
		correctAnswer: "Donald Trump",
	...
	{
		question: "We want to innovate and we will win someday.",
		correctAnswer: "Kanye West",
		src: "#",
		//profileImg: "assets/images/kanye-west-profile.jpeg",
		twitterHandle: "@KanyeWest"
	...
	{
		question: "The doors may be heavier, but I promise you WE WILL BREAK THEM.",
		correctAnswer: "Kanye West",
		src: "#",
		//profileImg: "assets/images/kanye-west-profile.jpeg",
		twitterHandle: "@KanyeWest"
	...
	{
		question: "My twitter has become so powerful that I can actually make my enemies tell the truth.",
		correctAnswer: "Donald Trump",
	...
	{
		question: "Yes I am personally rich and I can buy furs and houses for my family",
		correctAnswer: "Kanye West",
		src: "#",
		//profileImg: "assets/images/kanye-west-profile.jpeg",
		twitterHandle: "@KanyeWest"
	...
	{
		question: "President Obama - close down the flights from Ebola infected areas right now, before it is too late! What the hell is wrong with you?",
		correctAnswer: "Donald Trump",
	...
	{
		question: "I have no interest in working with anyone who is too important or too good or too traditional to take a call at 3am",
		correctAnswer: "Kanye West",
		src: "#",
		//profileImg: "assets/images/kanye-west-profile.jpeg",
		twitterHandle: "@KanyeWest"
	...
	{
		question: ".@katyperry Katy, what the hell were you thinking when you married loser Russell Brand. There is a guy who has got nothing going, a waste!",
		correctAnswer: "Donald Trump",
	...
	{
		question: "I'd like to address the false stories and noise that have been engineered by the media.",
		correctAnswer: "Kanye West",
		src: "#",
		//profileImg: "assets/images/kanye-west-profile.jpeg",
		twitterHandle: "@KanyeWest"
...
var triviaGame = {
	...
	getGif: function(){
		...
		var search = "q=";
		var celebrity = triviaGame.questionBank[x].correctAnswer.replace(regex, "+");
		var limit = "&limit=100"
		...
		$.ajax(this.href, {
			...
			triviaGame.questionBank[x].gifURL = response.data[randomGif].images.original.url;
		})
	},
	endGame: function(){
		...
		if(!this.gameData.bonusRound){
			$("#content,blockquote,#question,button,.buttons").hide();
	...
	setQuestion: function(){
		x = Math.floor(Math.random() * this.questionBank.length);
		$("#question").text(this.questionBank[x].question);
		this.getGif();
		var idNumber = 1;
		for(i = 0; i < 4; i++){
			selectionNumber = Math.floor(Math.random() * this.questionBank[x].answers.length);
			var selectId = "#selection" + idNumber;
			idNumber = idNumber + 1;
			$(selectId).text(this.questionBank[x].answers[selectionNumber]);
			$(selectId).val(this.questionBank[x].answers[selectionNumber]);
			this.questionBank[x].answers.splice([selectionNumber],1);
		}
	},
	showMessage: function() {
		...
		$("#link,#twitterHandle a").attr({href:this.questionBank[x].src, target:"_blank"});
		$("#twitterHandle a").text(this.questionBank[x].twitterHandle)
		$("#profileLogo").css("background-image", "url(" + this.questionBank[x].profileImg + ")");
		if(this.gameData.game === "win"){
			...
			$("#gameMessage2").html("<h6>The correct answer was: <br /><strong>" + this.questionBank[x].correctAnswer + "</strong><h6>");
			if(this.volume === true){
	...
	showGIF: function() {
		...
		if(!this.gameData.bonusRound){
			$("#gif").show();
			$("#gifURL").attr("src",this.questionBank[x].gifURL);
			setTimeout(function(){
				...
				if (timer === 0) {
					...
					triviaGame.showMessage();
						setTimeout(function(){
							triviaGame.stop();
							triviaGame.showGIF();
							triviaGame.questionBank.splice([x],1);
							triviaGame.run();						
						}, 3000);						
					}
				else{
	...
	selectedAnswer: function(obj){
		...
		if (buttonSelected.val() === this.questionBank[x].correctAnswer){
			triviaGame.stop();
	...
	bonusQuestion: function(){
		...
		var y = Math.floor(Math.random() * bonusQuestions.length);
		$("#bonusQuestion").text(bonusQuestions[y].question);
		var determineValue = bonusQuestions[y].correctAnswer
	...
	bonusRound: function(){
		...
		if(this.volume === true){
			var delayAudio = 0;
			if($("#question").text().match("JIMMY") && this.volume === true){
				bonusHead = "kanye";
		...
		var length = bonusQuestions.length;
		...
		$("#bonusRound").append('<div id="bonus" style="position:relative;"></div>');
		$("#bonus").append('<div id="bonusTrump" style="position:absolute;z-index:1;left:5%;bottom:25%"></div>');
		$("#bonusTrump").append('<button id="trumpButton" class="trumpButton btn" onclick="triviaGame.bonusClick(this)"></button>');
		$("#bonusRound").append('<blockquote id="bonusTweet" style="display:inline-block" class="twitter-tweet"></blockquote>');
		$("#bonusTweet").append('<header class="text-left"><div id="bonusProfileLogo" class="mb-2 mt-1 d-inline rounded-circle text-left bonusProfileLogo"></div><h6 id="bonusHandle" class="twitterHandle mx-auto pl-2 mb-2 mt-1 d-inline text-left"></h6><div class="text-right"><button id="bonusTime" type="button" class="btn btn-sm btn-primary ml-4 mb-2 mt-1 d-inline">Time: <span id="bonusTimeRemaining" class="badge badge-light ml-4 mb-2 mt-1 d-inline">15</span></button></div></header><br /><p class="text-left" id="bonusQuestion"></p>');
		$("#bonus").append('<div id="bonusKanye" style="position:absolute;z-index:1;right:5%;bottom:25%"></div>');		
		$("#bonusKanye").append('<button id="kanyeButton" class="kanyeButton btn" onclick="triviaGame.bonusClick(this)"></button>');
		$("#bonus").hide();
		$("#bonusTweet").hide();
		this.bonusQuestion();
	...
	reset: function() {
		...
		$("#twitterSong").get(0).play(); 
		$("#start,#results").hide();
		$("#content,#time,.multipleChoice,#content,#content,blockquote,#question,.buttons").show();
